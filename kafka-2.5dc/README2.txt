...
- [Schema Registry HA testing](#schema-registry-ha-testing)
...
## Schema Registry HA testing

Schema Registry runs in **both** regions against the one stretched Kafka cluster, sharing a `group.id`, both `leader.eligibility=true` (single-primary HA — see [Cross-region Schema Registry](#configuration)). This staged plan verifies failover. Run the destructive stages in a non-production environment first, and use your actual SR port/service names (8081 assumed).

> The stages are ordered so each builds on the last — verify the topology is correct before failing anything over. Stage 4 (SR down, brokers up) and Stage 5 (brokers down) test **different** layers; keep them separate.

### Stage 0 — Preconditions

Confirm the HA topology exists before testing failover, or you'll test the wrong thing.

```bash
# SR pods exist in BOTH regions
oc get pods -n kafka-region-a | grep -i schema
oc get pods -n kafka-region-b | grep -i schema

# both share the SAME group id and are leader-eligible
oc get schemaregistry -n kafka-region-a -o yaml | grep -iE "group.id|eligibility"
oc get schemaregistry -n kafka-region-b -o yaml | grep -iE "group.id|eligibility"

# _schemas is RF=4, spread 2+2 across regions, full ISR
oc exec <broker-pod> -n kafka-region-a -- \
  kafka-topics --bootstrap-server localhost:9071 --describe --topic _schemas
```

**Pass:** SR pod in each region; identical `group.id`; both `leader.eligibility=true`; `_schemas` shows RF=4, replicas spanning 110/210-range brokers, ISR=4.

### Stage 1 — Identify the current primary

```bash
curl -s http://schemaregistry.kafka-region-a.svc.cluster.local:8081/v1/metadata/id
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/v1/metadata/id
```

Record which instance is primary (expect region A in steady state).

### Stage 2 — Baseline (both regions healthy)

Prove reads and writes work through **both** SR endpoints before any failure.

```bash
# WRITE via region A (primary) — should succeed
curl -s -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
  --data '{"schema":"{\"type\":\"record\",\"name\":\"HATest\",\"fields\":[{\"name\":\"id\",\"type\":\"string\"}]}"}' \
  http://schemaregistry.kafka-region-a.svc.cluster.local:8081/subjects/ha-test-value/versions

# READ back via region B — should return the same schema
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/ha-test-value/versions/latest

# WRITE via region B (secondary) — should also succeed (forwarded to primary)
curl -s -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
  --data '{"schema":"{\"type\":\"record\",\"name\":\"HATest2\",\"fields\":[{\"name\":\"id\",\"type\":\"string\"}]}"}' \
  http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/ha-test2-value/versions
```

**Pass:** write via A succeeds; read via B returns the same schema (shared store); write via B succeeds (secondary forwards to primary).

### Stage 3 — SR pod failure (single instance)

Test SR-level HA first — kill the **primary** SR pod only.

```bash
oc delete pod <region-a-schema-pod> -n kafka-region-a

# reads via region B must keep working
watch -n1 'curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/ha-test-value/versions/latest'

# who's primary now?
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/v1/metadata/id
```

**Pass:** reads via region B never fail; region B is elected primary; when region A's SR pod restarts it rejoins as secondary. Isolates "SR failover" from "region failover".

### Stage 4 — Region A SR failure (the core HA goal)

Scale SR to zero in region A (cleaner than killing brokers — tests the SR-serving-in-B path with Kafka still healthy).

```bash
oc scale statefulset <region-a-schema-sts> -n kafka-region-a --replicas=0   # or deployment

# reads via region B — must succeed
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/ha-test-value/versions/latest
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects

# region B should now be primary
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/v1/metadata/id
```

**Pass:** all reads via region B succeed; region B is now primary. Proves a surviving SR in region B serves schemas when region A is gone.

### Stage 5 — Writes during a true region-A outage

This is where the **RF=4 / minISR=3** choice shows up. Take down region-A **brokers** (not just SR) so `_schemas` drops to its 2 region-B replicas, below minISR.

```bash
oc scale statefulset <region-a-kafka-sts> -n kafka-region-a --replicas=0

# _schemas now has only 2 ISR (region B) — below minISR=3
oc exec <region-b-broker-pod> -n kafka-region-b -- \
  kafka-topics --bootstrap-server localhost:9071 --describe --topic _schemas

# READ via region B — should still WORK
curl -s http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/ha-test-value/versions/latest

# WRITE a NEW schema via region B — EXPECTED TO FAIL (minISR not met)
curl -s -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
  --data '{"schema":"{\"type\":\"record\",\"name\":\"OutageTest\",\"fields\":[{\"name\":\"id\",\"type\":\"string\"}]}"}' \
  http://schemaregistry.kafka-region-b.svc.cluster.local:8081/subjects/outage-test-value/versions
```

**Pass (important expectation):** reads succeed, but the new-schema write **fails** (not-enough-replicas / timeout). This is **correct** for this design — it confirms read continuity, not write continuity, during a region loss. Testing it now means no surprises in a real incident.

### Stage 6 — Recovery

```bash
oc scale statefulset <region-a-kafka-sts> -n kafka-region-a --replicas=2
oc scale statefulset <region-a-schema-sts> -n kafka-region-a --replicas=1

# _schemas ISR returns to 4
oc exec <broker-pod> -n kafka-region-a -- \
  kafka-topics --bootstrap-server localhost:9071 --describe --topic _schemas

# writes work again via either endpoint
curl -s -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
  --data '{"schema":"{\"type\":\"record\",\"name\":\"RecoveryTest\",\"fields\":[{\"name\":\"id\",\"type\":\"string\"}]}"}' \
  http://schemaregistry.kafka-region-a.svc.cluster.local:8081/subjects/recovery-test-value/versions
```

**Pass:** ISR back to 4; writes succeed again; both SRs healthy (one primary, one secondary); all pre-outage schemas still present (no data loss).

### What each stage proves

| Stage | Failure tested | What it proves |
|---|---|---|
| 2 | None (baseline) | Primary-secondary wiring; shared store |
| 3 | Primary SR pod dies | SR-level auto-failover |
| 4 | Region-A SR gone | Region B serves schemas when A is down (core HA goal) |
| 5 | Region-A brokers gone | Reads survive; writes blocked by minISR=3 (accepted trade-off) |
| 6 | Recovery | Clean re-convergence, no data loss |
