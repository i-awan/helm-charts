...
| `nodeSelector` (region node labels) | skipped — not needed on a single test cluster | intended to apply, but **currently a no-op on CRD versions that reject `spec.podTemplate.nodeSelector` as an unknown field** — see the quorum/CRD-compatibility section below before relying on this for real fault isolation |
| Pod anti-affinity | soft (`preferred...`) when `nodeSelector.enabled: false` — a 1-2 node test cluster may have nowhere else to schedule | hard (`required...`) when `nodeSelector.enabled: true` — real fault isolation across nodes matters in production |
| `region.mode` | `full` (renders `KRaftController` + `Kafka`; `SchemaRegistry`/`Schema` CRs only render when `mode: full`, so the 0.5DC correctly gets none of them) | `full` for region-a/b, `light` for the 0.5DC (renders only `KRaftController` — no `Kafka`, no `SchemaRegistry`, no `Schema` CRs) |
...
different region's values file. What actually ties those three separate
releases into *one* Kafka cluster is **not** Helm, Kubernetes, or a
shared `clusterID` alone — see "Cross-namespace/cross-region KRaft
quorum" further down for what's actually required; the earlier framing
here undersold it.
...
- **Always check `helm install`/`upgrade` output for `Warning: unknown field`
  ...
  recognize gets silently dropped, not rejected. The install succeeds,
  the pod runs, and the value you set simply never takes effect. Hit
  this three separate times on the same cluster (`controllerQuorumVoters`,
  `podTemplate.initContainers`, `podTemplate.nodeSelector`) — see the
  quorum section further down for the full story and the working
...
- **A single boolean flag driving two unrelated Kubernetes mechanisms
  ...
  even one values file (this happened to the mock region files at one
  point) silently reintroduces hard anti-affinity and unschedulable pods
  on a small test cluster. Worth grepping all values files for the
  setting after any change to this logic, rather than assuming it
...
This is the single most important finding from validating the mock
multi-region setup, and it changes a claim made earlier in this document
— worth reading even if you skip everything else below.

**Matching `cluster.clusterID` across regions is NOT sufficient to form
one quorum.** It was treated earlier in this README as the mechanism
that ties three regions into one logical cluster. It isn't — it's a
necessary label, but nowhere close to sufficient. Proof, captured live
on a real cluster:
...
silent-rejection pattern: `spec.podTemplate.initContainers` (the
Schema Registry "wait-for-kafka" init container never actually ran —
its crash-loop-then-recover behavior was masking that the whole time)
and `spec.podTemplate.nodeSelector` (node-label-based pod scheduling
doesn't currently work on this CRD version either, regardless of the
`nodeSelector.enabled` setting — see the updated caveat in "One chart,
two deployment shapes" above).
...
```
This is what `templates/kraftcontroller.yaml` actually renders now —
built automatically from `staticQuorumVoters` via a Helm range, not
something you write by hand.

**Practical implication if you hit this on your own cluster:** always
check `helm install`/`upgrade` output for `Warning: unknown field` lines.
A structured CR field silently doing nothing is far more dangerous than
an outright error — the install "succeeds," the pod runs, and the actual
...
into a multi-voter static list via a config change alone. The working
path was a clean wipe (`helm uninstall` + delete PVCs across all three
namespaces) and a fresh simultaneous bootstrap with the static voter list
already in place from the start — not the earlier "region-a first, fetch
clusterID, propagate" dance, which is specific to the dynamic-quorum
observer/promote model this chart is deliberately *not* using.
...
```
Followed by: `orders3` (3 partitions, RF=3, min.insync.replicas=2)
created successfully across all 3 brokers; every partition's `Replicas:`
showing `110,111,210`; and a record produced via `kafka-avro-console-producer`
